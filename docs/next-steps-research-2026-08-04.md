...
| Cities with a centre within 10 / 25 / 50 miles | **184 / 224 / 227** |
| Cities with a *comprehensive* centre within 10 / 25 / 50 miles | 147 / 213 / 223 |
| Median distance from a city to its nearest centre | **4.0 miles** |
| Program pages that could carry a real office within 25 miles | **3,234 of 3,266 (99.0%)** |
| Centres with a published phone number and opening hours | 183 / 183 |
...
- **Some training is outside the ETPL entirely.** On-the-job training, customized training,
  ...
  those, so the site must not imply that the ETPL is the whole of what a job centre can fund.

### 1.3 Who decides, and on what
...
may be made available to adults and dislocated workers who a centre determines are:
...
### 1.4 Priority is a legal requirement, and it runs one way
...
([20 CFR 1010.200](https://www.ecfr.gov/current/title-20/section-1010.200)), and centres are
...
the priority only operates if the centre knows. That is a step, not a disclaimer.

### 1.5 WIOA money is the last money in, not the first
...
Centres "must consider the availability of other sources of grants … such as Temporary
...
### 1.6 There is money beyond tuition, and most people do not know to ask
...
### 1.7 The obligation to fund has one explicit escape clause
...
one-stop system. Federal rules distinguish a *comprehensive* centre — "a physical location
...
### 1.9 What varies by local area — measured, not asserted

Three California boards' published ITA policies, read on 2026-08-04:
...
A **2× spread** in the cap between two boards, a third whose number is not written down where
a jobseeker could find it, a lifetime rather than per-course limit in one area, and an
explicit residency priority in another. This is the concrete content of "it varies", and it is
why the site can describe the mechanism but must never quote a number.

---

## Part 2 — The centre locator
...
case-insensitive — `/v1/AJCFinder/…` answers identically — so capitalisation was never the
problem. Two sibling endpoints also answer: `/v1/ajcfinder/{userId}` (every centre in the
country, 2,146 of them) and `/v1/ajcfinder/{userId}/{jobCenterId}` (one centre's detail).
...
or a bare state code. A state code returns the whole state: `CA` gives 183 centres, and every
California centre found by probing border ZIP codes was already in that set. So **one request
...
A border ZIP search returns out-of-state centres (Blythe's nearest are in Arizona). That is
correct, not a bug — but they are not centres that can open a *California* ITA, so anything
published for a Californian filters on state. The module documents this and exposes the field
to filter on.

**The per-centre detail endpoint is deliberately not used.** It carries a "Language
Capability" field that would matter more to this project's Spanish-speaking readers than
almost anything else on the page — but in a random ten-centre sample, nine were blank. 183
...
| Within | Cities with any centre | Cities with a comprehensive centre |
|---|---|---|
...
| Within | Program pages with any centre | With a comprehensive centre |
|---|---|---|
| 10 miles | 2,897 / 3,266 (88.7%) | 2,595 (79.5%) |
| 25 miles | **3,234 / 3,266 (99.0%)** | 3,142 (96.2%) |
| 50 miles | 3,266 / 3,266 (100%) | 3,221 (98.6%) |

The eight worst-served cities, nearest centre in miles: Coalinga 38.7, South Lake Tahoe 36.5,
...
different facts: "we could not check" is not "California has no job centres", and a consumer
...
Twelve questions, in `local_help.QUESTIONS`, each carrying the rule that makes it worth
asking. They are split by who can answer, because sending someone to a job centre to ask about
...
3. *What exactly do I hold at the end, who issues it, and does an employer or licensing board
   recognise it?* A listed program must lead to a credential, employment, or measurable
   progress toward one (20 CFR 680.420) — and a school's certificate of completion and a state
   licence are very different things to be holding.
4. *If I stop partway, what do I owe?* An ITA is a payment agreement and may be paid in
   instalments (20 CFR 680.300), so the answer involves the provider and the centre together,
...
**To the America's Job Center** — seven questions:
...
8. *What is the most this area will put into an ITA, and would that cover this program?* Caps
   are local: $7,000 in San Diego, $14,000 lifetime in Imperial, unpublished in San
   Bernardino.
...
12. *What should I bring, and how long does a determination take?* The determination rests on
    an interview, evaluation or assessment and career planning that the centre must document
    (680.220); there is no federal minimum waiting period, so the answer is local.

A thirteenth is worth adding to the page but not to a person's mouth: **say if you receive
public assistance, are on a low income, are basic skills deficient, or are a veteran or an
eligible spouse.** The priority only operates if the centre knows (680.600, 1010.300).
...
2. **The nearest one to three America's Job Centers**, by name, address, phone and hours, with
   the comprehensive ones labelled. Phone first: it is the only contact channel populated for
...
> each sets its own policies, so the answer can differ between two people in neighbouring
...
Three specific things the copy must never do:
...
- **Never quote a dollar cap.** They range at least 2× across the state and one of the three
  boards checked does not publish one at all.
- **Never imply the ETPL is the whole of what a centre can fund.** On-the-job and customized
...
- **Publishing the per-centre "Language Capability" field.** Nine of ten sampled were blank.
- **Fetching centres per city at build time.** 227 requests where 1 will do.
...
- **Cadence.** Centre records carry a `LastUpdated` date, so a quarterly refresh alongside the
...
- **Naming the local board.** The most useful single fact — *which* of the 45 areas a person
  is in — is not in the centre records. EDD's coordinator table names a contact per area, and
...
| 20 CFR part 678 (one-stop delivery system) | `https://www.ecfr.gov/current/title-20/part-678` | 2026-08-04 | Comprehensive vs affiliate centres |
...
| CareerOneStop Web API — AJCFinder (source D6) | `https://api.careeronestop.org/v1/ajcfinder/...` | 2026-08-04 | 183 California centres |
...
*Note on verification:* the regulation text quoted here was read from the eCFR API
(`/api/versioner/v1/full/{date}/title-20.xml?part=…`), not from a search summary. The local
board policies were read from the boards' own PDFs.
