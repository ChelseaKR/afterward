...
def problems_in(programs: list[dict[str, Any]]) -> list[str]:
    ...
    Separate from :func:`problems` so the same two rules can be asked of records that never
    came off this disk -- a dataset fetched from the live site, or one held in memory by a
    test -- without going back through a file path.
    """
