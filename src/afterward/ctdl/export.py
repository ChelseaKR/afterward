...
def ctdl_coverage_problems(
    ...
    """Every figure or claim in a coverage statement that the export or the source contradicts.

    Three comparisons, and only the second is a check of the arithmetic.
    ...
    The third is :func:`unprojected_claim_problems`, which checks the statement's prose
    rather than its figures: a property the reasons call dropped that the document emits
    anyway, or a property on a ``ceterms:LearningProgram`` that nothing counts.
    """
